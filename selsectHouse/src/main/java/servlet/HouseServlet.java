...
public class HouseServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch(method){
			...
			case "shouSellTime2": Housequery2(request,response) ;request.getRequestDispatcher("/afterView/shenheshouloufabushijian.jsp").forward(request,response) ;break;
			case "shouSellTime3": Housequery(request,response) ;request.getRequestDispatcher("/afterView/guanliyuan-shenheshoulouxinxifabu.jsp").forward(request,response) ;break;
	...
	private void Housequery2(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		List<HouseSource> houseSources=houseSourceDao.selectInfoNopass();
	...
	private void Housequery1(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
	...
	private void houseAdd(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		String end = MyUTF.getNewString(request.getParameter("endTime").toString());
		String start1 = MyUTF.getNewString(request.getParameter("starTime1").toString());
		String end1= MyUTF.getNewString(request.getParameter("endTime1").toString());
		String name = MyUTF.getNewString(request.getParameter("name")) ;
		 
		System.out.println("发布摇号开始时间"+start+"发布摇号结束时间"+end+"  id: "+name);
		boolean istrue =   houseSourceDao.verifyInfo(name, start, end,start1,end1);
		if(istrue) {
